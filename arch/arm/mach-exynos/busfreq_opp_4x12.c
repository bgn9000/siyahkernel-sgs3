...
ssize_t store_bus_mV_table(struct kobject *kobj,
...
static struct global_attr bus_mV_table_attr = __ATTR(bus_mV_table,
		0644, show_bus_mV_table, store_bus_mV_table);
...
int exynos4x12_init(struct device *dev, struct busfreq_data *data)
{
	...
	if (sysfs_create_file(cpufreq_global_kobject, &bus_mV_table_attr.attr))
		pr_err("Failed to create sysfs file(mV_table)\n");
